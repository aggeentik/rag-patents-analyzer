...
**Entity lookup modes (controlled via `GRAPH_SEMANTIC_ENTITY_SEARCH`):**

| Mode | Env var | How entities are matched in KG |
|---|---|---|
| Exact / pattern (default) | `false` | SQL `LIKE` query — fast, no embeddings required |
| Semantic | `true` | Cosine similarity between query entity embedding and stored entity embeddings — requires embeddings to be pre-built |

The default (`false`) is safe to use without any additional setup. Set `GRAPH_SEMANTIC_ENTITY_SEARCH=true` only after entity embeddings have been built in the knowledge graph database.
