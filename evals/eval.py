...
def initialize_retrievers(
    ...
    logger.info(f"✓ Graph retriever loaded")
...
def run_evaluation(
    ...
    for idx, test_case in enumerate(dataset["test_cases"], 1):
        ...
        try:
            ...
            logger.info(f"Retriever stats: BM25={stats['bm25_hits']}, "
...
def calculate_ragas_metrics(results: Dict[str, Any], llm_client: LLMClient = None, ragas_model: str = None) -> Dict[str, float]:
    """
    ...
    try:
        ...
        if ragas_model:
            ...
            if ragas_model.startswith("gpt-") and not ragas_model.startswith("gpt-4o"):
                ...
                llm = ChatLiteLLM(model=ragas_model)
            else:
            ...
            llm = ChatLiteLLM(model=llm_client.model)
        else:
            # Fallback to OpenAI
            from langchain_openai import ChatOpenAI
        ...
        logger.info("Computing faithfulness, answer_relevancy, context_precision, context_recall...")
            ...
            if hasattr(evaluation_results, 'to_dict'):
                ...
                for key, value in result_dict.items():
                    if isinstance(value, dict) and len(value) == 1:
                        ...
                        metrics_dict[key] = float(list(value.values())[0])
...
def generate_report(
    ...
    for category, stats in category_stats.items():
        count = stats["count"]
        if count > 0:
            stats["avg_contexts"] = stats["avg_contexts"] / count
            stats["avg_bm25_hits"] = stats["avg_bm25_hits"] / count
            stats["avg_semantic_hits"] = stats["avg_semantic_hits"] / count
            stats["avg_graph_hits"] = stats["avg_graph_hits"] / count
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate Patents Analyzer RAG system using RAGAS"
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument(
        "--skip-ragas",
        action="store_true",
        help="Skip RAGAS metrics calculation"
    )
    parser.add_argument(
        "--ragas-model",
        default=None,
    ...
    logger.info(f"Loaded {len(dataset['test_cases'])} test cases across "
